...
# Инициализация структуры таблиц и применение патчей на лету
with app.app_context():
    db.create_all()
    try:
        db.session.execute(db.text("ALTER TABLE mailing_task ADD COLUMN IF NOT EXISTS sent_count INTEGER DEFAULT 0;"))
        db.session.execute(db.text("ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_ends TIMESTAMP;"))
        db.session.commit()
    except Exception:
        db.session.rollback()

# --- Вспомогательные Асинхронные методы работы с API Telegram ---
...
async def _get_chats(session_string):
    ...
    try:
        if not await client.is_user_authorized():
            raise Exception("Сессия этого аккаунта аннулирована.")
...
# --- Адаптивный Синий Дизайн интерфейса ---
BASE_TEMPLATE = """
<!DOCTYPE html>
<html lang="ru">
<head>
...
<body class="min-h-screen flex flex-col justify-between antialiased">
    ...
    <footer class="border-t border-slate-900 bg-slate-950/80 py-4 text-center">
        <p class="text-xs text-slate-500">Vest Traffer &copy; 2026. Поддержка: <a href="https://t.me/VestTraffSupport" class="text-blue-400 underline">@VestTraffSupport</a></p>
    </footer>
    {% endif %}
...
def register():
    if request.method == 'POST':
        ...
        # Автоматическое начисление 3 бесплатных дней подписки новым юзерам
        ...
        db.session.commit()
        flash('Вы успешно зарегистрированы! Доступно 3 дня бесплатного теста.')
...
# --- Панель управления с 3 вкладками ---
@app.route('/dashboard')
def dashboard():
    ...
    # ВКЛАДКА 2: ФУНКЦИИ (С началом выбирается аккаунт - Этап 1)
    ...
    # ВКЛАДКА 3: ПРОФИЛЬ (Расчет прайса Crypto Bot)
    else:
        html = f"""
        <div class="space-y-6">
...
def create_mailing_form():
    ...
    <script>
        ...
        function goToStep(s) {
            if (s === 2) { let c = document.querySelectorAll('.chat-checkbox:checked').length; if (c === 0 || c > 50) { alert("Ошибка: Выберите от 1 до 50 диалогов."); return; } }
...
# --- Модуль добавления аккаунтов Telethon ---
