...
# Жесткая конфигурация базы данных PostgreSQL и Telegram API
...
# --- Асинхронные Сервисы Telethon (Выполняются на стороне сайта) ---
...
async def _get_chats(session_string):
    ...
    try:
        if not await client.is_user_authorized():
            raise Exception("Сессия этого аккаунта невалидна или была принудительно завершена.")
...
# --- Единый Глобальный Шаблон (Премиум Неоново-Синий) ---
BASE_TEMPLATE = """
<!DOCTYPE html>
<html lang="ru">
<head>
...
<body class="min-h-screen flex flex-col justify-between antialiased">
    ...
    <footer class="border-t border-slate-900 bg-slate-950/80 py-6 px-4 text-center">
        <div class="container mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <p class="text-sm text-slate-500 mx-auto md:mx-0">&copy; 2026 <span class="text-blue-500 font-bold">Vest Traffer</span>. Поддержка: <a href="https://t.me/VestTraffSupport" target="_blank" class="text-blue-400 underline hover:text-blue-300 font-bold">@VestTraffSupport</a></p>
        </div>
    </footer>
...
def index():
    html = """
    <div class="text-center my-12 max-w-3xl mx-auto">
        ...
        <p class="text-slate-400 text-lg mb-8">Менеджмент безлимитных сеток аккаунтов, пошаговые алгоритмы массовой рассылки (ЛС, группы, каналы) и автоматизированные триггерные автоответчики.</p>
...
def dashboard():
    ...
    <div class="flex flex-col gap-8 w-full">
        ...
        <section class="p-6 rounded-2xl premium-card shadow-xl">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-slate-900">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Подключение аккаунтов через Telethon и управление кампаниями</p>
                </div>
        ...
        <section class="p-6 rounded-2xl premium-card shadow-xl">
            <h2 class="text-xl font-black text-blue-400 uppercase tracking-wider mb-4">🤖 Модуль Автоответчиков</h2>
            {% if not responders %}
                <p class="text-slate-500 text-sm italic">Нет активных конфигураций автоответа.</p>
            {% else %}
                <div class="space-y-3">
                    {% for r in responders %}
                    <div class="p-4 rounded-xl bg-slate-950/40 border border-slate-900 flex justify-between items-center text-sm gap-4">
                        <div>
                            <span class="text-xs uppercase font-bold text-slate-500">Триггер: </span>
        ...
        <section class="p-6 rounded-2xl premium-card shadow-xl">
            <h2 class="text-xl font-black text-blue-400 uppercase tracking-wider mb-6">⚡ Журнал и Прогресс Рассылок</h2>
            <div class="space-y-4">
                {% if not tasks %}
                    <p class="text-slate-500 text-sm italic">История кампаний пуста.</p>
                {% endif %}
                {% for t in tasks %}
                <div class="p-4 rounded-xl bg-slate-950 border border-slate-900 space-y-3">
                    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-2 text-xs sm:text-sm">
                        ...
                        <span class="text-slate-400">Успешно доставлено: <strong class="text-blue-400">{{ t.sent_count }}</strong> из <strong class="text-slate-300">{{ t.total_messages }}</strong></span>
                    </div>
                    ...
                    <details class="text-xs text-slate-500 group cursor-pointer">
                        <summary class="hover:text-slate-300 font-medium select-none py-1">Показать параметры и выбранные ID чатов</summary>
                        <div class="mt-2 p-3 bg-slate-900/60 rounded-xl space-y-2 text-slate-400">
                            <p><strong>Алгоритм:</strong> {% if t.mailing_type == 'simultaneous' %}Одновременный{% else %}Рандомный{% endif %} | <strong>Задержка:</strong> {{ t.delay }} секунд</p>
                            <p class="text-slate-500 font-semibold uppercase text-[10px] tracking-wider">Пул ID получателей:</p>
                            <div class="max-h-24 overflow-y-auto font-mono text-[11px] bg-slate-950/80 p-2 rounded-lg text-blue-300 divide-y divide-slate-900">
                                {{ t.chats }}
                            </div>
                        </div>
                    </details>
...
def autoresponder_setup():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash("Автоответчик успешно инициализирован фоновым воркером!")
        return redirect(url_for('dashboard'))
        
    html = """
    <div class="max-w-xl mx-auto premium-card p-6 sm:p-8 rounded-3xl shadow-2xl">
        <h2 class="text-xl font-black text-blue-400 mb-6 uppercase tracking-wider">⚙️ Настройка Автоответчика</h2>
        <form method="POST" class="space-y-4">
            <div>
                <label class="block text-xs font-bold uppercase text-slate-400 mb-2">Область реагирования триггера</label>
                <select name="trigger_type" class="w-full rounded-xl px-4 py-3 text-white text-sm">
                    ...
                    <option value="all">Все входящие сообщения (ЛС + группы)</option>
                </select>
            </div>
            <div>
                <label class="block text-xs font-bold uppercase text-slate-400 mb-2">Ключевые слова</label>
                <input type="text" name="keywords" value="-" required class="w-full rounded-xl px-4 py-3 text-sm">
                <p class="text-[10px] text-slate-500 mt-1">Перечислите через запятую. Если оставить одиночный знак <strong class="text-blue-400">-</strong>, автоответчик сработает на абсолютно любое сообщение.</p>
            </div>
            <div>
                <label class="block text-xs font-bold uppercase text-slate-400 mb-2">Текст ответа</label>
                <textarea name="reply_text" rows="4" placeholder="Ваш готовый скрипт ответа..." required class="w-full rounded-xl px-4 py-3 text-sm"></textarea>
            </div>
            <button type="submit" class="w-full py-3.5 rounded-xl bg-blue-600 text-white font-bold text-sm uppercase tracking-wider blue-glow">Запустить автоответчик</button>
        </form>
    </div>
    """
    return render_page(html)
...
def autoresponder_delete(rid):
    ...
    db.session.delete(r)
    db.session.commit()
    flash("Автоответчик успешно остановлен.")
    return redirect(url_for('dashboard'))
    ...
    <div class="premium-card p-5 sm:p-8 rounded-3xl shadow-2xl max-w-4xl mx-auto mt-4">
        ...
        <form method="POST" id="wizard-form" class="space-y-6">
            ...
            <div id="step-3" class="space-y-4 hidden">
                ...
                <div>
                    <textarea name="message" rows="5" placeholder="Введите ваш продающий оффер..." required class="w-full rounded-xl px-4 py-3 text-white text-sm"></textarea>
                </div>
                <div class="flex justify-between pt-6 border-t border-slate-900">
                    ...
                    <button type="submit" class="px-8 py-3.5 rounded-xl bg-blue-600 font-black text-white uppercase tracking-wider text-xs shadow-xl blue-glow hover:bg-blue-500 transition-all">Запустить кампанию</button>
                </div>
            </div>
