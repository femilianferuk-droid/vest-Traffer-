...
# --- Изолированные асинхронные функции для работы с Telethon ---
...
# --- Главный Базовый Шаблон (Премиум Неон) ---
BASE_TEMPLATE = """
<!DOCTYPE html>
<html lang="ru">
<head>
...
def dashboard():
                ...
                {% else %}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="p-4 rounded-xl bg-slate-950 border border-slate-800 flex justify-between items-center">
                            ...
                            <a href="/mailing/load_chats?account_id={}" class="text-xs bg-purple-950 border border-purple-500 text-purple-300 px-3 py-1.5 rounded-lg hover:bg-purple-500 hover:text-white transition">Запустить рассылку</a>
                        </div>
                        {% endfor %}
                    </div>
                {% endif %}
    ...
    """
    # Заполнение динамических списков внутри строки
    acc_rows = ""
    for acc in accounts:
        acc_rows += f"""
        <div class="p-4 rounded-xl bg-slate-950 border border-slate-800 flex justify-between items-center">
    ...
    task_rows = ""
    for t in tasks:
        m_type = "Одновременный" if t.mailing_type == "simultaneous" else "Рандомный"
        task_rows += f"""
        <tr class="border-b border-slate-800/50 hover:bg-slate-950/40">
            <td class="py-3 px-4 font-mono text-slate-300">#{t.id}</td>
            <td class="py-3 px-4">{m_type}</td>
            <td class="py-3 px-4 text-purple-400 font-semibold">{t.total_messages}</td>
            <td class="py-3 px-4 font-mono">{t.delay} сек.</td>
            <td class="py-3 px-4">
                <span class="px-2 py-0.5 rounded text-xs border bg-slate-900">{t.status}</span>
    ...
    final_html = html.replace('{% for acc in accounts %}{% endfor %}', acc_rows).replace('{% for t in tasks %}{% endfor %}', task_rows)
    # Очистка заглушек подстановки для безопасности рендеринга
    final_html = final_html.format("", "") if "{}" in final_html else final_html
    
    # Прямая чистая сборка контента
    dashboard_template = BASE_TEMPLATE.replace(
        '{% block content %}{% endblock %}', 
        html.replace('{% if not accounts %}\n                    <p class="text-slate-500">У вас пока нет добавленных Telegram-аккаунтов.</p>\n                {% else %}\n                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">\n                        {% for acc in accounts %}\n                        <div class="p-4 rounded-xl bg-slate-950 border border-slate-800 flex justify-between items-center">\n                            <div>\n                                <p class="font-mono text-cyan-400">{{ acc.phone }}</p>\n                                <span class="text-xs text-green-400 bg-green-950/40 px-2 py-0.5 rounded border border-green-500/30">Активен</span>\n                            </div>\n                            <a href="/mailing/load_chats?account_id={{ acc.id }}" class="text-xs bg-purple-950 border border-purple-500 text-purple-300 px-3 py-1.5 rounded-lg hover:bg-purple-500 hover:text-white transition">Запустить рассылку</a>\n                        </div>\n                        {% endfor }\n                    </div>\n                {% endif %}', acc_rows if accounts else '<p class="text-slate-500">У вас пока нет добавленных Telegram-аккаунтов.</p>').replace('{% for t in tasks %}\n                            <tr class="border-b border-slate-800/50 hover:bg-slate-950/40">\n                                <td class="py-3 px-4 font-mono text-slate-300">#{{ t.id }}</td>\n                                <td class="py-3 px-4">{% if t.mailing_type == \'simultaneous\' %}Одновременный{% else %}Рандомный{% endif %}</td>\n                                <td class="py-3 px-4 text-purple-400 font-semibold">{{ t.total_messages }}</td>\n                                <td class="py-3 px-4 font-mono">{{ t.delay }} сек.</td>\n                                <td class="py-3 px-4">\n                                    <span class="px-2 py-0.5 rounded text-xs border bg-slate-900">{{ t.status }}</span>\n                                </td>\n                            </tr>\n                            {% endfor %}', task_rows)
    )
...
def create_mailing_form():
    ...
    mailing_template = BASE_TEMPLATE.replace(
        '{% block content %}{% endblock %}',
        html.replace('{% for c in chats %}\n                        <label class="flex items-center gap-3 p-2 bg-slate-900/60 rounded-lg hover:bg-slate-900 cursor-pointer text-sm">\n                            <input type="checkbox" name="selected_chats" value="{{ c.id }}" class="rounded text-purple-600 bg-slate-950 border-slate-800">\n                            <span class="truncate text-slate-200">{{ c.title }}</span>\n                        </label>\n                        {% endfor %}', chat_checkboxes)
    )
    return render_template_string(mailing_template)
